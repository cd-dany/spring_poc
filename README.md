...
This is the type of view a student can expect from Spring. Their syllabi is fully parsed into a calendar view and able to see the dates. As previously stated, this is a proof of concept, not all features and ideas were implemented in this concept. This is achieved by using Jackson JSON Mapper Library in Java. We take a JSON file that represents a course syllabi and use the JSON Mapper to convert from JSON to Java Objects we can use to display the information.


## Limitations with Similar Tools
While there are some tools like <a href = "https://www.syllabuddy.com">SyllaBuddy</a> that aim to help with this problem. When given a PDF that is unorganized like the following: ![SyllaBuddy Output](img/syllabuddy.png "Sylla Buddy Output") SyllaBuddy struggles to accurately and effectively output correct data unless the syllabi are somewhat organized or a typical syllabi.
...
This is the simplest form of the Spring pipeline. The idea here is to parse the user's input or in this case their syllabi into data the AI model can work with (text). Next the AI tokenizes the input and outputs a formatted JSON/XML file that conforms to a JSON/XML Schema that represents how the output should be organized. We can use this to validate the output from the AI model and it also gives the chance to create a "Standard" for how we represent a course syllabi.
